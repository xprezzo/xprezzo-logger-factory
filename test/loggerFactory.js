...
process.env.NO_DEPRECATION = 'loggerFactory'
...
describe('loggerFactory()', function () {
  ...
  describe('tokens', function () {
    ...
    describe(':response-time', function () {
      ...
      it('should be empty if loggerFactory invoked after response sent', function (done) {
        ...
        var logger = loggerFactory(':response-time', {
          immediate: true,
          stream: stream
        })
    ...
    describe(':total-time', function () {
      ...
      it('should be empty if loggerFactory invoked after response sent', function (done) {
        ...
        var logger = loggerFactory(':total-time', {
          immediate: true,
          stream: stream
        })
        ...
        request(server)
          ...
          .expect(102, function (err, res) {
            if (err && err.code === 'ECONNRESET') {
              ...
              // loggerFactory needs to test in this condition even if
...
describe('loggerFactory.compile(format)', function () {
  describe('arguments', function () {
    describe('format', function () {
      it('should be required', function () {
        assert.throws(loggerFactory.compile.bind(loggerFactory), /argument format/)
      })

      it('should reject functions', function () {
        assert.throws(loggerFactory.compile.bind(loggerFactory, function () {}), /argument format/)
      })

      it('should reject numbers', function () {
        assert.throws(loggerFactory.compile.bind(loggerFactory, 42), /argument format/)
      })

      it('should compile a string into a function', function () {
        var fn = loggerFactory.compile(':method')
...
function createRequestListener (format, opts, fn, fn1) {
  var logger = loggerFactory(format, opts)
